...
int main(int argc, char** argv) {
     ...
     * TRG BITS
     */
    const int bit_em_inBending_Tot = 0; // The inbending trigger bit for all sectors
    const int bit_em_inBending_Sec = 1; // The 1st bit for the sector based inbending trigger bit
    const int bit_em_NoDC_Tot = 7; // The NoDC electron trigger bit for all sectors
    const int bit_em_NoDC_Sec = 8; // The NoDC electron trigger bit for all sectors
    ...
    const double ESum_AboveThresholdCut = 0.25;
    const double Nphe_AboveThresholdcut = 2;
    ...
    const double ECin_UMin_cut = 2.5; // For oubendings this doesn't matter much;
    ...
    const double ECout_UMin_cut = 2; // For oubendings this doesn't matter much;
    ...
    TFile *file_out = new TFile(Form("Validate_Electron_%d.root", run), "Recreate");
    ...
    const int N_MaxEventToProcess = 2000000;
    int evCounter = 0;
    int nEvPID11 = 0;
    try {
        ...
        while (reader.next() == true) {
            ...
            if( evCounter > N_MaxEventToProcess ){
                break;
            }
...
//                }
                ...
                for (int i_vtp = 0; i_vtp < v_vtp_time.size(); i_vtp++) {
                    ...
                    bool em_tot_trig = v_vtp_trbit.at(i_vtp)(bit_em_NoDC_Tot, bit_em_NoDC_Tot);
                    bool em_sect_trig = v_vtp_trbit.at(i_vtp)(bit_em_NoDC_Sec, bit_em_NoDC_Sec + n_sect);
                    bool em_DC_inbend_trig = v_vtp_trbit.at(i_vtp)(bit_em_inBending_Sec, bit_em_inBending_Sec + n_sect);
                    int em_sect_trig_int = v_vtp_trbit.at(i_vtp)(bit_em_NoDC_Sec, bit_em_NoDC_Sec + n_sect);

                    for (int iS = 0; iS < n_sect; iS++) {

                        if (v_vtp_trbit.at(i_vtp)(iS + bit_em_NoDC_Sec, iS + bit_em_NoDC_Sec)) {

                            if (35 + 0.25 * t_HTCC < v_vtp_time.at(i_vtp) < 55 + 0.25 * t_HTCC) {
