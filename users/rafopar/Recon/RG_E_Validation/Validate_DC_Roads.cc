...
int main(int argc, char** argv) {
    ...
    if (argc > 1) {
        run = atoi(argv[1]);
        //sprintf(inputFile, "Data/single_0%d.hipo", run);
    ...
    const int bit_em_inBending_Tot = 7; // The inbending trigger bit for all sectors
    const int bit_em_inBending_Sec = 8; // The 1st bit for the sector based inbending trigger bit
    const int bit_em_NoDC_Tot = 0; // The NoDC electron trigger bit for all sectors
    const int bit_em_NoDC_Sec = 1; // The NoDC electron trigger bit for all sectors
    const int bit_em_OutBending_Tot = 14; // The OutBending trigger bit for all sectors
    const int bit_em_OutBending_Sec = 15; // The 1st bit for the sector based Outbending trigger bit

    
    const double Eb = 10.547;
    ...
    const double ESum_Threshold = 0.2;
    ...
    const double ECout_UMax_cut = 35.;
    ...
    TFile *file_out = new TFile(Form("Validate_DC_Roads_Run_%d.root", run), "Recreate");
    ...
    TH1D *h_Q2_All_em1 = new TH1D("h_Q2_All_em1", "", 200, 0., 5.5);
    TH1D *h_Q2_Good_em1 = new TH1D("h_Q2_Good_em1", "", 200, 0., 5.5);
    TH1D *h_Q2_passed_em1 = new TH1D("h_Q2_passed_em1", "", 200, 0., 5.5);
    TH1D *h_Q2_missed_em1 = new TH1D("h_Q2_missed_em1", "", 200, 0., 5.5);
    ...
    /**
     * Histograms with different energy cuts
     */

    const int bit_29 = 536870912; // bit 29
    const int bit_27 = 134217728; // bit 27
    const int bit_26 = 67108864; // bit 26

    TH1D * h_p_em0 = new TH1D("h_p_em0", "", 200, 0., 1.05 * Eb);
    TH1D * h_p_em1 = new TH1D("h_p_em1", "", 200, 0., 1.05 * Eb);
    TH1D * h_p_em2 = new TH1D("h_p_em2", "", 200, 0., 1.05 * Eb);
    TH1D * h_p_em3 = new TH1D("h_p_em3", "", 200, 0., 1.05 * Eb);
    TH1D * h_p_em4 = new TH1D("h_p_em4", "", 200, 0., 1.05 * Eb);

    TH1D *h_W0 = new TH1D("h_W0", "", 200, 0.6, 3.6);
    TH1D *h_W1 = new TH1D("h_W1", "", 200, 0.6, 3.6);
    TH1D *h_W2 = new TH1D("h_W2", "", 200, 0.6, 3.6);
    TH1D *h_W3 = new TH1D("h_W3", "", 200, 0.6, 3.6);

    TH1D *h_xB0 = new TH1D("h_xB0", "", 200, 0., 1.2);
    TH1D *h_xB1 = new TH1D("h_xB1", "", 200, 0., 1.2);
    TH1D *h_xB2 = new TH1D("h_xB2", "", 200, 0., 1.2);
    TH1D *h_xB3 = new TH1D("h_xB3", "", 200, 0., 1.2);

    TH1D *h_Q2_0 = new TH1D("h_Q2_0", "", 200, 0., 3.5);
    TH1D *h_Q2_1 = new TH1D("h_Q2_1", "", 200, 0., 3.5);
    TH1D *h_Q2_2 = new TH1D("h_Q2_2", "", 200, 0., 3.5);
    TH1D *h_Q2_3 = new TH1D("h_Q2_3", "", 200, 0., 3.5);


    int evCounter = 0;

    const int nMaxEventsToProcess = 4000000;
    ...
    try {
        ...
        while (reader.next() == true) {
            ...
            em_WithDCBits[0] = TS_TrgWord & 128 ? true : false;
            ep_WithDCBits[0] = TS_TrgWord & 16384 ? true : false; // 128 is bit 7
            em_NoDCBits[0] = TS_TrgWord & 1 ? true : false; // 16384 is bit 14
            ...
            for (int ibit = 0; ibit < 6; ibit++) {
                long secBitWithDC = TMath::Power(2, ibit + bit_em_inBending_Sec);
                long secBitWithDCep = TMath::Power(2, ibit + bit_em_OutBending_Sec);
                long secBitNoDC = TMath::Power(2, ibit + bit_em_NoDC_Sec);
            ...
            bool bit_Ecut1 = bit_29&TS_TrgWord;
            bool bit_Ecut2 = bit_27&TS_TrgWord;
            bool bit_Ecut3 = bit_26&TS_TrgWord;

            if (n_emFD > 0) {
                double th_em = acos(v_em.at(0).pz() / v_em.at(0).p()) * TMath::RadToDeg();

                double nue = Eb - v_em.at(0).p();
                double Q2 = 2 * Eb * v_em.at(0).p()*(1 - v_em.at(0).pz() / v_em.at(0).p());
                double W = sqrt(Mp * Mp + 2 * Mp * nue - Q2);
                double xB = Q2 / (2 * Mp * nue);

                h_Q2_All_em1->Fill(Q2);

                for (int iSec = 0; iSec < n_sect; iSec++) {

                    if (em_RecInSec[iSec] && em_NoDCBits[iSec + 1]) {

                        h_Q2_Good_em1->Fill(Q2);
                        h_p_em0->Fill(v_em.at(0).p());
                        h_W0->Fill(W);
                        h_xB0->Fill(xB);
                        ...
                        if (bit_Ecut1) {
                            h_p_em1->Fill(v_em.at(0).p());
                            h_W1->Fill(W);
                            h_xB1->Fill(xB);
                            h_Q2_1->Fill(Q2);
                        }
                        if (bit_Ecut2) {
                            h_p_em2->Fill(v_em.at(0).p());
                            h_W2->Fill(W);
                            h_xB2->Fill(xB);
                            h_Q2_2->Fill(Q2);
                        }
                        if (bit_Ecut3) {
                            h_p_em3->Fill(v_em.at(0).p());
                            h_W3->Fill(W);
                            h_xB3->Fill(xB);
                            h_Q2_3->Fill(Q2);
                        }
