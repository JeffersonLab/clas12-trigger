...
int main(int argc, char** argv) {
    ...
    const int DC_RoadOutbendBit = 128; // 7th bit
    const int DC_RoadInbendBit = 1; // 1st bit
    const int e_noDCRoad = 16384; // 14th bit
    ...
    const double ESum_Threshold = 0.25;
    ...
    TFile *file_out = new TFile(Form("Validate_DC_Roads_WithTSBitsTorusOutbend_10p55_Run_%d.root", run), "Recreate");
    ...
    TH2D *h_th_P_HasTrgBit_em1 = new TH2D("h_th_P_HasTrgBit_em1", "", 200, 0., Eb, 200, 0., 45);
    ...
    TH1D *h_Q2_All_em1 = new TH1D("h_Q2_All_em1", "", 100, 0., 8.5);
    TH1D *h_Q2_HasTrgBit_em1 = new TH1D("h_Q2_HasTrgBit_em1", "", 100, 0., 8.5);
    TH1D *h_Q2_Good_em1 = new TH1D("h_Q2_Good_em1", "", 200, 0., 8.5);
    TH1D *h_Q2_passed_em1 = new TH1D("h_Q2_passed_em1", "", 200, 0., 8.5);
    TH1D *h_Q2_missed_em1 = new TH1D("h_Q2_missed_em1", "", 200, 0., 8.5);
    ...
    TH1D *h_vt_em_has_emTrgBit1 = new TH1D("h_vt_em_has_emTrgBit1", "", 200, 70., 130.);
    TH1D *h_vz_em_has_emTrgBit1 = new TH1D("h_vz_em_has_emTrgBit1", "", 200, -20., 20.);
    TH1D *h_vt_em_hasNo_emTrgBit1 = new TH1D("h_vt_em_hasNo_emTrgBit1", "", 200, 70., 130.);
    TH1D *h_vz_em_hasNo_emTrgBit1 = new TH1D("h_vz_em_hasNo_emTrgBit1", "", 200, -20., 20.);

    TGraph *gr_beamCharge = new TGraph();

    int evCounter = 0;

    const int nMaxEventsToProcess = 200000;

    double chargeMax = 0.;
    double chargeMin = 1.e15;
    ...
    try {
        ...
        while (reader.next() == true) {
            ...
            double beamCharge = double(bRecEvent.getFloat("beamCharge", 0));

            chargeMax = beamCharge > chargeMax ? beamCharge : chargeMax;
            chargeMin = beamCharge < chargeMin ? beamCharge : chargeMin;
            ...
            bool e_NoDCBits[n_sect + 1] = {0}; // Trg bits for e- w/o DR_Roads. 0 for Allsec, 1,2,3,4,5,6 for each sec
            bool inbending_WithDCBits[n_sect + 1] = {0}; // Trg bits for e- w DR_Roads. 0 for Allsec, 1,2,3,4,5,6 for each sec
            bool outbending_WithDCBits[n_sect + 1] = {0}; // Trg bits for e- w DR_Roads. 0 for Allsec, 1,2,3,4,5,6 for each sec
            ...
            for (int ipart = 0; ipart < nPart; ipart++) {
                ...
                int sect_particle = recp.secPCal() - 1;
            ...
            if (TS_TrgWord & DC_RoadInbendBit) {
            ...
            h_bit_7VS0->Fill((TS_TrgWord & DC_RoadInbendBit) > 0, (TS_TrgWord & DC_RoadOutbendBit) > 0);

            //cout<<( (TS_TrgWord & DC_RoadInbendBit) > 0)<<"    "<<( (TS_TrgWord & DC_RoadOutbendBit) > 0)<<endl;

            if (TS_TrgWord & DC_RoadOutbendBit) {
            ...
            if (n_emFD > 0) {
                h_n_em_bits_0AND7->Fill((TS_TrgWord & DC_RoadInbendBit) > 0, (TS_TrgWord & DC_RoadOutbendBit) > 0);
            }
            ...
            inbending_WithDCBits[0] = TS_TrgWord & DC_RoadInbendBit ? true : false;
            outbending_WithDCBits[0] = TS_TrgWord & DC_RoadOutbendBit ? true : false; // DC_RoadOutbendBit is bit 7
            e_NoDCBits[0] = TS_TrgWord & 16384 ? true : false; // 16384 is bit 14
            ...
            for (int ibit = 0; ibit < 6; ibit++) {
                ...
                inbending_WithDCBits[ibit + 1] = TS_TrgWord & secBitWithDC;
                outbending_WithDCBits[ibit + 1] = TS_TrgWord & secBitWithDCep;
                e_NoDCBits[ibit + 1] = TS_TrgWord & secBitNoDC;
            }

            if (n_emFD > 0) {
                double th_em = acos(v_em.at(0).pz() / v_em.at(0).p()) * TMath::RadToDeg();
                double Q2_em = 2 * Eb * v_em.at(0).p()*(1 - cos(v_em.at(0).th() * TMath::DegToRad()));
                double vt = v_em.at(0).vt();
                double vz = v_em.at(0).vz();
                
                h_Q2_All_em1->Fill(Q2_em);
                for (int iSec = 0; iSec < n_sect; iSec++) {

                    if (em_RecInSec[iSec]) {
                        if (outbending_WithDCBits[iSec + 1]) {
                            h_vt_em_has_emTrgBit1->Fill(vt);
                            h_vz_em_has_emTrgBit1->Fill(vz);
                        } else {
                            h_vt_em_hasNo_emTrgBit1->Fill(vt);
                            h_vz_em_hasNo_emTrgBit1->Fill(vz);
                        }
                    }

                    if (em_RecInSec[iSec] && outbending_WithDCBits[iSec + 1]) {
                        h_Q2_HasTrgBit_em1->Fill(Q2_em);
                        h_th_P_HasTrgBit_em1->Fill(v_em.at(0).p(), th_em);
                    }

                    if (em_RecInSec[iSec] && e_NoDCBits[iSec + 1]) {

                        h_Q2_Good_em1->Fill(Q2_em);
                        h_th_P_Allem2->Fill(v_em.at(0).p(), th_em);
                        h_vz_em2->Fill(v_em.at(0).vz());
                        if (outbending_WithDCBits[iSec + 1]) {
                            h_Q2_passed_em1->Fill(Q2_em);
                            h_DC_RoadBitsets1->Fill(1);
                        } else {
                            h_Q2_missed_em1->Fill(Q2_em);
            ...
            if (n_epFD > 0) {
                ...
                for (int iSec = 0; iSec < n_sect; iSec++) {
                    if (ep_RecInSec[iSec] && e_NoDCBits[iSec + 1]) {
                        ...
                        if (inbending_WithDCBits[iSec + 1]) {
    ...
    gr_beamCharge->SetPoint(0, 0, chargeMin);
    gr_beamCharge->SetPoint(1, 1, chargeMax);
    gr_beamCharge->SetPoint(2, 2, chargeMax - chargeMin);
    gr_beamCharge->Write("gr_beamCharge");
    gDirectory->Write();
