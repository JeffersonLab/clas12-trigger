...
//by  b_branchname->GetEntry(ientry); //read only this branch
   ...
   TH2D *h_n_HTCC_trg_good_em_P1 = new TH2D("h_n_HTCC_trg_good_em_P1", "", 200, 0., 1.2*Eb, 5, -0.5, 4.5);
   ...
   TH2D *h_n_PCalEC_trg_good_em_P1 = new TH2D("h_n_PCalEC_trg_good_em_P1", "", 200, 0., 1.2*Eb, 5, -0.5, 4.5);
   TH2D *h_n_PCalEC_trg_good_em_P2 = new TH2D("h_n_PCalEC_trg_good_em_P2", "", 200, 0., 1.2*Eb, 5, -0.5, 4.5);

   TH2D *h_HTCC_vtp_em_sect1 = new TH2D("h_HTCC_vtp_em_sect1", "", 7, -0.5, 6.5, 7, -0.5, 6.5);
   TH2D *h_PCalEC_vtp_em_sect1 = new TH2D("h_PCalEC_vtp_em_sect1", "", 7, -0.5, 6.5, 7, -0.5, 6.5);
   

   TH1D *h_P_em1 = new TH1D("h_P_em1", "", 200, 0.02*Eb, 1.2*Eb);
   TH1D *h_P_neg1 = new TH1D("h_P_neg1", "", 200, 0.02*Eb, 1.2*Eb);

   TH1D *h_P_Good_em1 = new TH1D("h_P_Good_em1", "", 200, 0.02*Eb, 1.2*Eb);
   TH1D *h_P_Good_em_trg_thresholds1 = new TH1D("h_P_Good_em_trg_thresholds1", "", 200, 0.02*Eb, 1.2*Eb);
   TH1D *h_P_Good_em_trg_thresholds_tr_found1 = new TH1D("h_P_Good_em_trg_thresholds_tr_found1", "", 200, 0.02*Eb, 1.2*Eb);
   TH1D *h_P_Good_em_trg_thresholds_NoDCRoad1 = new TH1D("h_P_Good_em_trg_thresholds_NoDCRoad1", "", 200, 0.02*Eb, 1.2*Eb);
   TH1D *h_P_Good_em_missed1 = new TH1D("h_P_Good_em_missed1", "", 200, 0.02*Eb, 1.2*Eb);
   TH1D *h_P_Good_em_missed_trg_thresholds1 = new TH1D("h_P_Good_em_missed_trg_thresholds1", "", 200, 0.02*Eb, 1.2*Eb);
   TH1D *h_P_Good_noEC1 = new TH1D("h_P_Good_noEC1", "", 200, 0.02*Eb, 1.2*Eb);
   TH1D *h_P_Good_noEC_trg_thresholds1 = new TH1D("h_P_Good_noEC_trg_thresholds1", "", 200, 0.02*Eb, 1.2*Eb);
   TH1D *h_P_Good_noEC_missed1 = new TH1D("h_P_Good_noEC_missed1", "", 200, 0.02*Eb, 1.2*Eb);
   TH1D *h_P_Good_noEC_missed_trg_thresholds1 = new TH1D("h_P_Good_noEC_missed_trg_thresholds1", "", 200, 0.02*Eb, 1.2*Eb);

   TH1D *h_P_Good_em0 = new TH1D("h_P_Good_em0", "", 200, 0.02*Eb, 1.2*Eb);
   TH1D *h_P_Good_em_DC_Missed0 = new TH1D("h_P_Good_em_DC_Missed0", "", 200, 0.02*Eb, 1.2*Eb);
   TH1D *h_P_Good_em_trg_thresholds_DC_Missed0 = new TH1D("h_P_Good_em_trg_thresholds_DC_Missed0", "", 200, 0.02*Eb, 1.2*Eb );
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
	...
	// ======= Selecting only negatives
        if( REC_Particle_charge->at(i_part) >= 0 ) {continue;}

	double Px =double(REC_Particle_px->at(i_part));         double Py = double(REC_Particle_py->at(i_part));        double Pz = double(REC_Particle_pz->at(i_part));
	...
	// ======================== HTCC =======================
        if( ind_HTCC[i_part] >= 0 ){
          ...
          phi_HTCC = HTCC_rec_phi->at(ind_HTCC_rec)*r2d + 30.; if( phi_HTCC < 0.) {phi_HTCC = phi_HTCC + 360.;};
	...
	if( ind_ECin[i_part] < 0 && ind_ECout[i_part] < 0 && ind_PCal[i_part] >= 0 && ind_HTCC[i_part] >= 0 && PCal_pass && nphe > 2 ){
        ...
        if( !( Good_Electron || Good_noEC ) ) {continue;}
	...
	for( int i_vtp = 0; i_vtp < v_vtp_time.size(); i_vtp++ ){
	  ...
	  bool em_DC_inbend_trig = v_vtp_trbit.at(i_vtp)(7, 12);
          int em_sect_trig_int = v_vtp_trbit.at(i_vtp)(1, 6);
	  bool PCalEC_trg = v_vtp_trbit.at(i_vtp)(13, 18);
	  bool HTCC_trg = v_vtp_trbit.at(i_vtp)(7, 12);

	  if(PCalEC_trg){
	    n_em_VTP_trig_PCalEC_allsec = n_em_VTP_trig_PCalEC_allsec + 1;
	  }
	  ...
	  for( int iS = 0; iS < n_sect; iS++){
	    ...
	    if( v_vtp_trbit.at(i_vtp)(iS+13, iS+13) ){
	      h_PCalEC_vtp_em_sect1->Fill(sec_em, iS);
	      n_em_VTP_trig_PCalEC_[iS] = n_em_VTP_trig_PCalEC_[iS] + 1;
	    }
	...
	if( Good_Electron ){
	  ...
	  h_n_PCalEC_trg_good_em_P1->Fill(P, n_em_VTP_trig_PCalEC_[sec_em]);
	...
	if( n_em_VTP_trig_[REC_Calorimeter_sector->at(ind_PCal[i_part]) - 1] == 0 ){
	  ...
	  // =============== Good electron was not found by trigger
          ...
          if( Good_Electron && n_em_VTP_trig_Good_em_[REC_Calorimeter_sector->at(ind_PCal[i_part]) - 1] == 0 ){
	      ...
	      h_n_PCalEC_trg_good_em_P2->Fill(P, n_em_VTP_trig_PCalEC_[sec_em]);
